// const Items = require("../models/shopItemModel");  //why?
...
customerController.newCard = async (req, res) => {
  const { customerId, itemID, quantityRequested } = req.body;
  try {
    const item = await Card.findOne({ itemId: itemID });
    if (!item || item.quantity < quantityRequested) {
      return res.status(400).json({ message: "Not enough items in inventory" });
    }
    // Decrement the quantity in the inventory
    await Card.updateOne({ itemId: itemID }, { $inc: { quantity: -quantityRequested } });
